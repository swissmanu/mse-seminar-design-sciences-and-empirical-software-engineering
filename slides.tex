...
\note[itemize]{
	...
	\item Transfer a topic to another area (frontend engineering)
...
\note[itemize]{
	\item Our own goals are formalized by a total of 8 research Questions
...
\note[itemize]{
	\item Quick suymmary on the most important terminology
}

\subsection*{Empirical Software Engineering}
\begin{frame}{Empirical Software Engineering}
	\begin{itemize}
		\item \textbf{Lack of} scientific and analytical \textbf{formalism}\bigskip
		\item \textbf{Apply empirical} research \textbf{methods} instead
	\end{itemize}
\end{frame}

\note[itemize]{
	\item Software engineering lacks formalism
	\item No clear "correct" or "wrong" way of doing software engineering
	\item Idea: Apply empirical research principles
	\item Example: Reasearch Cycle: Plan - Do - Check ... Agile anyone?\bigskip
	\item This brings us to Design sciences:
}
...
\note[itemize]{
	\item A "meta" research framework
	\item Design a research method
	\item Research on an artifact on a specific context
	\item Evaluate answers, check stakeholder requirements\bigskip
	\item Reiterate until stakeholders satisfied
}
...
\note[itemize]{
	\item Wilkinson
	\item A checklist to maximise the value an outcome of a research project\bigskip
	\item Allow results to be found (keywords, unique identifiers, ...)
	\item Make things easily accessible (archives, librarires, tech stack)
	\item Use formats accepted in a specific expert group, provide schema information
	\item Provide as much information as possible so results can be reproduced\bigskip
	\item Hasselbring: Specific recommendations for software engineering research
	\item Example: Versioning Software Objects along with Research Report
}
	...
	\begin{columns}[t, onlytextwidth]
		\column{0.5\textwidth}
			\begin{itemize}
				...
				\item Originated in \textbf{functional} programming\bigskip
...
\note[itemize]{
	\item Reactive programming is a declarative programming paradigm
	\item Describe what to do, not "how" to do it
	\item Originated in functional programming, first appeared with Haskell\bigskip
	\item We describe values, how they depend on each other
	\item And how they are transformed over time
	\item RP usually requires a runtime environment establishing consistency of values
}
