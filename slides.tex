...
\note[itemize]{
	\item Welcome To My Talk titled:
	\item Reactive Programming in Frontend Engineering Today
}
...
\note[itemize]{
	\item Second: \textbf{Empirical Software engineering}:
	\item Definition of \textbf{RP and OOP}?
	\item When use \textbf{RP}?
...
\note[itemize]{
	\item Lets begin now with the first part
	\item Salvaneschi et al based on \textbf{thesis}:
}
...
\note[itemize]{
	\item Developed four research questions:
}
...
\note[itemize]{
	\item RP impact on \textbf{correctness} of comprehension?
	\item Differences in how much \textbf{time} to comprehend RP?
	\item higher/lower \textbf{skill level} to correctly understand?
	\item Is there a \textbf{reason} in potential differences?\bigskip
	...
	\begin{itemize}
		...
		\item Survey to capture personal opinion afterwards
	\end{itemize}
}
